...
void save_image_array(uint8_t* image_array) {
 ...
 __device__ int* compress(uint8_t N1, uint8_t N2, uint8_t P, int* res_str) {
    int H, L;
    if (N1 > N2) {
        H = N1;
        L = N2;
...
__global__ void felics(uint8_t* image_array, int* output_array) {
    ...
    for (int i = tid; i < width * height; i += stride) {
        if (i % width == 0 || i % width == 1) {
            ...
            compress(image_array[i - 2], image_array[i - 1], image_array[i], *res_str); // Use the correct indices
            output_array[i] = res_str// Store the 35-bit result
        }
    }
}
